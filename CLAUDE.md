...
- **movement.ts** — Minecraft-like physics tick: gravity, jump velocity, ground/air drag, horizontal acceleration. Jumps clamp away downward velocity and add, so one taken while falling delivers full height and one taken while rising stacks. A mid-air second jump spends an air charge that landing restores; the jump cooldown runs purely on time and is the only rate limiter. Uses `MC_TICK = 0.05` as the reference timestep; all physics values scale by `dt/MC_TICK`. Two modes: physics movement (default) and freecam (`FREECAM` function, toggled via debug panel).
...
Non-voxel objects (enemies, future projectiles). Managed by `EntityManager` in `src/entity.ts`. Entities are defined by 5 composable axes: **Shape** (mesh geometry, e.g. Sphere), **Material** (texture + physical stats from the `materials` table), **Role** (AI strategy: Rush / Zone / Crush), **Size** (scale), **Traits** (bolt-on modifiers, currently empty). The material table is the primary tuning surface — read by rendering (texture layer, tile density), physics (restitution), and AI (baseSpeed). `mass` and `restitution` are cached on each Entity at spawn (mass = `density * size² / NORMALIZATION`) so hot paths don't re-look-up per frame.

Per-frame flow runs in **three passes** over the entity list: (1) `entityAITick` → `entityPhysicsTick` for each entity (AI writes velocity, physics integrates and resolves voxel + player collisions); (2) `resolveSpherePair` for each (i,j) sphere pair (mass-weighted depenetration + impulse response); (3) render-offset → `uploadTransform` for each entity. Splitting pair resolution into its own pass guarantees it sees finalized post-integration positions on both sides.
