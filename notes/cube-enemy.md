...
- **Tip duration.** Starts at ~0.3s per tip for a reference-size cube; controlled at the material level via `tipSpeed` (see Phase 4 notes). Linear easing. Ease-in/out deferred — easy swap inside `advanceCubeTip`.
...
- `tipRate` is the **aggression / eagerness to chase**. It shrinks the idle time between tips. Cranking it past the point where the cube is tipping continuously hits a ceiling at the `tipSpeed` cap — "tip more often" can't beat "each tip takes X seconds."

Parallel to sphere-only `baseSpeed`, but cubes need two knobs because discrete tipping has both a per-tip duration and an inter-tip gap, and those shape the feel independently (a snappy-but-hesitant cube vs a sluggish-but-relentless one are both expressible). These are the primary material-level tuning surface for cube difficulty.
